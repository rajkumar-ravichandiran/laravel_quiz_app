                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <h3 class="mb-0">{{ __('Edit Question - '.html_entity_decode($question->question, ENT_QUOTES)) }}</h3>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('questions.update',$question) }}" autocomplete="off">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="question" id="input-question" class="form-control form-control-alternative{{ $errors->has('question') ? ' is-invalid' : '' }}" placeholder="{{ __('Question') }}" value="{{ old('question',html_entity_decode($question->question, ENT_QUOTES)) }}" required autofocus>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="category_id" class="form-control{{ $errors->has('category_id') ? ' is-invalid' : '' }}" id="category_id"  required>
                                        ...
                                        <option {{old('category_id',$question->category_id) == $key?'selected' :'' }} value="{{$key}}">{{$category}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" id="type"  required>
                                        ...
                                        <option {{old('type',$question->type) == 0 ?'selected' :'' }} value="0">{{__('Multiple Choice')}}</option>
                                        <option {{old('type',$question->type) == 1 ?'selected' :'' }} value="1">{{__('True/False')}}</option>
                                    </select>
                                        @if ($errors->has('type'))
                                            <span class="invalid-feedback" style="display: block;" role="alert">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="difficulty" class="form-control{{ $errors->has('difficulty') ? ' is-invalid' : '' }}" id="difficulty"  required>
                                        ...
                                        <option {{old('difficulty',$question->level) == 1 ?'selected' :'' }} value="1">{{__('Easy')}}</option>
                                        <option {{old('difficulty',$question->level) == 2 ?'selected' :'' }} value="2">{{__('Medium')}}</option>
                                        <option {{old('difficulty',$question->level) == 3 ?'selected' :'' }} value="3">{{__('Hard')}}</option>
                                    </select>
                              ...
                              <div class="col-md-6">
                                    ...
                                    <textarea name="choices" class="form-control{{ $errors->has('choices') ? ' is-invalid' : '' }}" id="input-choices" rows="3">{{ old('choices',html_entity_decode(implode(',',json_decode($question->choices)), ENT_QUOTES)) }}</textarea>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="answer" class="form-control{{ $errors->has('answer') ? ' is-invalid' : '' }}" id="answer"  required>
                                        ...
                                        <option {{old('answer',$question->answer) == 0 ?'selected' :'' }} value="0">{{__('1')}}</option>
                                        <option {{old('answer',$question->answer) == 1 ?'selected' :'' }} value="1">{{__('2')}}</option>
                                        <option {{old('answer',$question->answer) == 2 ?'selected' :'' }} value="2">{{__('3')}}</option>
                                        <option {{old('answer',$question->answer) == 3 ?'selected' :'' }} value="3">{{__('4')}}</option>
                                    </select>
                              ...
                              <div class="col-md-6">
                                    ...
                                    <textarea name="summary" class="form-control{{ $errors->has('summary') ? ' is-invalid' : '' }}" id="input-summary" rows="3">{{ old('summary',$question->summary) }}</textarea>
...
$(document).ready(function() {
    ...
    if($('#type').val() === '1'){
        $('#answer option[value="2"]').prop('disabled', true);
        $('#answer option[value="3"]').prop('disabled', true);
        $('#input-choices').prop('readonly', true);
    }else{
        $('#answer option').prop('disabled', false);
        $('#input-choices').prop('readonly', false);
    }
});
